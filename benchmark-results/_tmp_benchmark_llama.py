...
LAMP_SERVER_URL = "http://localhost:3002"
...
def main():
    ...
    for i, test in enumerate(tests):
        ...
        if raw is None:
            ...
            results.append({"id": idx, "status": "ollama_error", "prompt": prompt})
            continue
        ...
        if parsed is None:
            ...
            results.append({"id": idx, "status": "json_error", "prompt": prompt, "raw": raw[:300]})
            if i < total - 1:
                time.sleep(pause)
            continue
